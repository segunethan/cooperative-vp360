...
const Login = () => {
  ...
  const { signIn, reloadTenant, signOut } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const tenant = await reloadTenant();
    setLoading(false);
    ...
    // No cooperative admin record for this account — never guess, just say so.
    await signOut();
    setError(
      `No cooperative admin account found for this email. Cooperative members sign in at ${MEMBER_APP_URL.replace(/^https?:\/\//, "")}.`
    );
  };
