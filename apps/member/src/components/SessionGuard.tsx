...
const INACTIVE_MS = 10 * 60 * 1000;  // show warning after 10 min
const COUNTDOWN_S = 60;                // 1-minute countdown before auto sign-out
...
export const SessionGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  const display = `0:${String(countdown).padStart(2, "0")}`;
    ...
    <>
      ...
      {showWarning && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl p-7 max-w-sm w-full text-center space-y-5 animate-in fade-in zoom-in-95 duration-200">
            <div className="relative w-16 h-16 mx-auto">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="font-mono font-bold text-base text-foreground">{display}</span>
              </div>
            </div>

            <div>
              <h2 className="text-lg font-bold text-foreground tracking-tight mb-1.5">Still there?</h2>
              <p className="text-sm text-muted-foreground leading-relaxed">
                You've been inactive for a while. Tap <strong>Stay signed in</strong> to continue.
              </p>
            </div>
